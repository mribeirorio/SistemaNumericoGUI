...
public class Calculo {
    ...
    public String primosAte(String op) throws Exception {
        // op = atkin ou op = simples
        Numeral numero = getNumero();
        ...
        if(numero.intValue()<=1) {
            ...
            String baseNumero = "";
            String titulo = "";
            String strTempoExecucao = "";
            try {
                //
                long startTime = System.nanoTime();
                if(op=="atkin") {
                    fatores = primosAte.getFatoresPrimosAteAtkin(numero.longValue());
                } else if(op=="eratostenes") {
                    fatores = primosAte.getFatoresPrimosAteEratostenes(numero.longValue());
                } else {
                ...
                /*if(durationNano > 1000000) {
                    strTempoExecucao = String.valueOf(durationNano / 1_000_000) + " milissegundos\n\n"; // Using underscore for readability
                } else {
                    strTempoExecucao = String.valueOf(durationNano) + " nanossegundos\n\n";
                }*/
                //
                BigDecimal tempoExecucao = new BigDecimal(durationNano).divide(new BigDecimal("1000000"));
                strTempoExecucao = tempoExecucao.toPlainString() + " milissegundos";
                //
                String strPrimos = fatores.toString();
                titulo = "Algoritmo " + (op=="atkin" ? "de Atkin\n" : (op=="eratostenes") ? "de Eratóstenes\n" : " simples\n");
                titulo += "Tempo gasto: "+strTempoExecucao+"\n\n";
                //
                if (!numero.getBaseNumerica().isBaseDecimal()) {
                    baseNumero = "Número informado: " + numero.toLiteral() + " (base " + numero.getBaseNumerica().getValorBase() + ")";
